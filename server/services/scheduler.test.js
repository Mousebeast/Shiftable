...
describe('fixedOnly mode', () => {
  const alice = { id: 1, name: 'Alice', min_hours_per_week: 0, max_hours_per_week: 40 };
  const bob = { id: 2, name: 'Bob', min_hours_per_week: 0, max_hours_per_week: 40 };
  const rule = { group_id: 10, day_of_week: 0, shift_template_id: 5, min_staff: 2, start_time: '11:00', hours: 5 };
  const aliceFixed = { user_id: 1, day_of_week: 2, shift_template_id: 7, group_id: 10, start_time: '17:00', hours: 6 };

  const inputs = {
    ...BASE,
    users: [alice, bob],
    userGroups: { 1: [10], 2: [10] },
    fixedSchedules: [aliceFixed],
    coverageRules: [rule],
  };

  it('returns only the fixed shift and skips the coverage fill', () => {
    const { shifts } = generateSchedule({ ...inputs, fixedOnly: true });
    expect(shifts).toHaveLength(1);
    expect(shifts[0].is_fixed).toBe(1);
    expect(shifts[0].user_id).toBe(1);
    expect(shifts[0].date).toBe('2026-06-04'); // day 2
  });

  it('the same inputs without fixedOnly do fill coverage', () => {
    // Guards against a false pass above: proves the coverage rule was live and
    // genuinely suppressed, not merely absent from the fixture.
    const { shifts } = generateSchedule(inputs);
    expect(shifts.length).toBeGreaterThan(1);
    expect(shifts.some(s => !s.is_fixed)).toBe(true);
  });

  it('emits no warnings even when everyone is under min_shifts', () => {
    // Phase 3 is skipped on purpose: in a deliberately partial schedule nearly
    // everyone is under minimum, so the warnings would restate the request.
    const { warnings } = generateSchedule({
      ...inputs,
      users: [{ ...alice, min_shifts_per_week: 5 }, { ...bob, min_shifts_per_week: 5 }],
      fixedOnly: true,
    });
    expect(warnings).toEqual([]);
  });

  it('keeps manual override shifts', () => {
    const { shifts } = generateSchedule({
      ...inputs,
      overrideShifts: [{
        user_id: 2, group_id: 10, shift_template_id: 9, date: '2026-06-05',
        start_time: '12:00', hours: 4, is_deleted: 0,
      }],
      fixedOnly: true,
    });
    expect(shifts).toHaveLength(2);
    expect(shifts.some(s => s.is_override === 1 && s.user_id === 2)).toBe(true);
  });

  it('still honours time off', () => {
    const { shifts } = generateSchedule({
      ...inputs,
      timeOffBlocks: { 1: ['2026-06-04'] },
      fixedOnly: true,
    });
    expect(shifts).toEqual([]);
  });

  it('still honours closed days', () => {
    const { shifts } = generateSchedule({
      ...inputs,
      closedDates: new Set(['2026-06-04']),
      fixedOnly: true,
    });
    expect(shifts).toEqual([]);
  });

  it('returns nothing when there are no fixed schedules at all', () => {
    const { shifts, warnings } = generateSchedule({ ...inputs, fixedSchedules: [], fixedOnly: true });
    expect(shifts).toEqual([]);
    expect(warnings).toEqual([]);
  });

  it('defaults to a full generate when fixedOnly is not passed', () => {
    const withFlag = generateSchedule({ ...inputs, fixedOnly: false }).shifts.length;
    const without = generateSchedule(inputs).shifts.length;
