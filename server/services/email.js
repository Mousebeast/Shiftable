...
async function sendWelcomeEmail(user, claimUrl) {
  ...
  try {
    await transporter.sendMail({
      ...
      html: `<p>Hi ${user.name},</p><p>Your account is ready. Click to claim it (expires in 72 hours):</p><p><a href="${claimUrl}">${claimUrl}</a></p>`,
    });
  } catch (err) {
...
async function sendPinResetEmail(user, claimUrl) {
  ...
  try {
    await transporter.sendMail({
      ...
      html: `<p>Hi ${user.name},</p><p>Use this link to reset your PIN (expires in 72 hours):</p><p><a href="${claimUrl}">${claimUrl}</a></p>`,
    });
  } catch (err) {
