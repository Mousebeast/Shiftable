  ...
  for (const rule of sortedRules) {
    ...
    const eligible = users
      .filter(u => {
        ...
        if (avail) {
          if (shiftStartMin < toMinutes(avail.start_time) || shiftEndMin > toMinutes(avail.end_time)) return false;
        }
        if (weeklyHours[u.id] + rule.hours > u.max_hours_per_week) return false;
        if (maxConsecutiveDays > 0) {
          if (consecutiveDaysEndingBefore(date, scheduledDates[u.id]) >= maxConsecutiveDays) return false;
        }
        return true;
      })
  ...
  for (const ec of eventCoverage) {
    ...
    const eligible = users
      .filter(u => {
        ...
        if (weeklyHours[u.id] + ec.hours > u.max_hours_per_week) return false;
        if (maxConsecutiveDays > 0) {
          if (consecutiveDaysEndingBefore(ec.date, scheduledDates[u.id]) >= maxConsecutiveDays) return false;
        }
        return true;
      })
      .sort((a, b) => weeklyHours[a.id] - weeklyHours[b.id]);
