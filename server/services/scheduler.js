...
function generateSchedule({ weekDates, maxConsecutiveDays, users, userGroups, fixedSchedules, overrideShifts = [], timeOffBlocks, availability, coverageRules, eventCoverage = [], closedDates = new Set(), dayPriorityOrder = [0,1,2,3,4,5,6], fillOrder = 'group_first' }) {
  const shifts = [];
  const warnings = [];
  ...
  for (const ec of eventCoverage) {
    ...
    const day_of_week = (ecDate.getUTCDay() + 6) % 7;
    allSlots.push({
      type: 'event',
      date: ec.date,
      day_of_week,
  ...
  for (const slot of allSlots) {
    const { date, day_of_week, group_id, shift_template_id, quota, start_time, hours } = slot;
