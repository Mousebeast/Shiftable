...
function setCookieAndRespond(res, user) {
  ...
  const nameSetting = db.prepare("SELECT value FROM app_settings WHERE key = 'restaurant_name'").get();
  const restaurantName = nameSetting?.value || 'Shiftable';
  return res.status(200).json({ user: { id: user.id, name: user.name, role: user.role }, restaurantName });
}
