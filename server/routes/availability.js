...
router.post('/', requireStaff, (req, res) => {
  ...
  // Validate effectiveFrom is a Monday
  const dayOfWeek = new Date(effectiveFrom + 'T00:00:00').getDay();
  if (dayOfWeek !== 1) {
    return res.status(400).json({ error: 'effectiveFrom must be a Monday' });
  }
  createAvailabilityEntries(
    req.user.userId,
    entries.map((e) => ({ ...e, effective_from: effectiveFrom })),
  );
...
router.put('/manager/:userId/:day', requireManager, (req, res) => {
  ...
  const { startTime, endTime, effectiveFrom } = req.body;
  if (!startTime || !endTime || !effectiveFrom) {
    return res.status(400).json({ error: 'startTime, endTime, effectiveFrom required' });
  }
  const dow = new Date(effectiveFrom + 'T00:00:00').getDay();
  if (dow !== 1) return res.status(400).json({ error: 'effectiveFrom must be a Monday' });
  upsertManagerAvailability(userId, dayOfWeek, startTime, endTime, req.user.userId, effectiveFrom);
  return res.json({ ok: true });
});
