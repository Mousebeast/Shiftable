...
appDb.exec(fs.readFileSync(path.join(__dirname, '../db/schema.sql'), 'utf8'));
const app = require('../index');
const { createTestDb, seedUser, seedGroup } = require('../test/helpers');

let sharedUser;
beforeAll(() => {
  sharedUser = seedUser(appDb, { email: 'shared@test.com', role: 'staff' });
});
...
describe('GET /api/schedule', () => {
  ...
  it('returns 200 with valid JWT cookie', async () => {
    const token = makeToken(sharedUser);
