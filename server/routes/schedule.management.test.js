...
describe('POST /api/schedule/generate', () => {
  ...
  it('mode=fixed-only seeds fixed shifts without filling coverage', async () => {
    db.prepare('INSERT INTO fixed_schedules (user_id, day_of_week, shift_template_id) VALUES (?, 2, ?)')
      .run(staff.id, template.id);
    const res = await request(app)
      .post('/api/schedule/generate')
      .set('Cookie', `token=${managerToken(manager.id)}`)
      .send({ week: WEEK, mode: 'fixed-only' });
    expect(res.status).toBe(201);
    expect(res.body.shifts).toHaveLength(1);
    expect(res.body.shifts[0].is_fixed).toBe(1);
    expect(res.body.shifts[0].date).toBe('2026-06-03'); // day 2 of the week
  });

  it('the same week generated in full mode does fill coverage', async () => {
    // Paired with the test above so a passing fixed-only result cannot be an
    // artefact of the fixture simply having nothing to fill.
    db.prepare('INSERT INTO fixed_schedules (user_id, day_of_week, shift_template_id) VALUES (?, 2, ?)')
      .run(staff.id, template.id);
    const res = await request(app)
      .post('/api/schedule/generate')
      .set('Cookie', `token=${managerToken(manager.id)}`)
      .send({ week: WEEK, mode: 'full' });
    expect(res.body.shifts.length).toBeGreaterThan(1);
  });
  ...
  it('returns 400 for an unknown mode', async () => {
    const res = await request(app)
      .post('/api/schedule/generate')
      .set('Cookie', `token=${managerToken(manager.id)}`)
      .send({ week: WEEK, mode: 'partial' });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/mode/);
  });

  it('fixed-only ignores a fixed schedule for a group the user has left', async () => {
    // Composes with the user_groups join added for the Sierra fix: a stale row
    // must not reappear just because this mode does nothing but fixed shifts.
    db.prepare('INSERT INTO fixed_schedules (user_id, day_of_week, shift_template_id) VALUES (?, 2, ?)')
      .run(staff.id, template.id);
    db.prepare('DELETE FROM user_groups WHERE user_id = ? AND group_id = ?').run(staff.id, group.id);
    const res = await request(app)
      .post('/api/schedule/generate')
      .set('Cookie', `token=${managerToken(manager.id)}`)
      .send({ week: WEEK, mode: 'fixed-only' });
...
describe('DELETE /api/schedule/shifts/:id', () => {
  ...
  it('removes shift and returns ok with violations', async () => {
    ...
    expect(Array.isArray(res.body.violations)).toBe(true);
    const row = db.prepare('SELECT id, is_deleted FROM schedule_shifts WHERE id=?').get(shiftId);
...
describe('POST /api/schedule/:id/shifts', () => {
  ...
  it('returns 409 when schedule is published', async () => {
    const { scheduleId } = seedPublishedWithShift();
    const res = await request(app)
      .post(`/api/schedule/${scheduleId}/shifts`)
      .set('Cookie', `token=${managerToken(manager.id)}`)
