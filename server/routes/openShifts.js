...
const { getUserWeeklyHours, getUserMaxHours } = require('../db/swaps');
...
router.post('/:id/claim', requireStaff, (req, res) => {
  ...
  const txResult = db.transaction(() => {
    ...
    // Weekly hour cap. Availability is deliberately not checked — claiming an
    // open shift is volunteering, and the claimer's own stated hours are
    // theirs to override. The restaurant's maximum is not, so it is enforced
    // here rather than surfaced as a warning: an open-shift claim has no
    // manager-review path, it either lands in the published schedule or it
    // does not. Inside the transaction so a concurrent claim cannot slip a
    // second shift past the cap between the read and the write.
    const maxHours = getUserMaxHours(userId, db);
    const currentHours = getUserWeeklyHours(userId, weekStart, db);
    if (currentHours + os.hours > maxHours) return 'over_hours';
  ...
  if (txResult === 'over_hours') return res.status(409).json({ error: 'This shift would put you over your maximum hours for the week' });
