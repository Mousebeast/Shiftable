...
const { localDateString } = require('../services/dates');
...
router.post('/', requireStaff, (req, res) => {
  ...
  // "Today" must resolve in the server's local timezone. toISOString() gave the
  // UTC date, so after ~7pm in a UTC- timezone a request for the actual current
  // day was rejected as being in the past.
  ...
  notifyApprovers({
    ...
    body: `${req.user.name || 'A staff member'} requested ${range}.`,
  });
