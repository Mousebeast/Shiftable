...
router.post('/:id/claim', requireStaff, (req, res) => {
  ...
  const data = { url: '/swaps' };
  if (autoApproved) {
    const reqTitle = 'Swap Approved';
    const reqBody = 'Your shift has been picked up — swap complete.';
    createNotification({ userId: swap.requester_id, type: 'swap_resolved', title: reqTitle, body: reqBody, data });
    sendToUser(swap.requester_id, { title: reqTitle, body: reqBody, data });
    const claimerTitle = 'Shift Picked Up';
    const claimerBody = 'Your swap claim was auto-approved — you\'re on for the shift.';
    createNotification({ userId: req.user.userId, type: 'swap_resolved', title: claimerTitle, body: claimerBody, data });
    sendToUser(req.user.userId, { title: claimerTitle, body: claimerBody, data });
  } else {
    const title = 'Swap Pending Review';
    const body = 'Your swap request needs manager approval.';
    createNotification({ userId: swap.requester_id, type: 'swap_resolved', title, body, data });
    sendToUser(swap.requester_id, { title, body, data });
    // The requester was told a manager has to look; until now nobody told the
    // manager. Excludes the claimer, who may be a manager claiming a shift.
    notifyApprovers({
      excludeUserId: req.user.userId,
      type: 'swap_pending',
      title: 'Swap Needs Review',
      body: `${req.user.name || 'A staff member'} claimed a swap that needs your approval.`,
    });
  }
...
router.delete('/:id', requireStaff, (req, res) => {
  const result = cancelSwap(Number(req.params.id), req.user.userId);
  if (result.changes === 0) return res.status(409).json({ error: 'Swap cannot be cancelled' });
...
router.patch('/:id/approve', requireManager, (req, res) => {
  ...
  if (swap.requester_id === req.user.userId || swap.claimer_id === req.user.userId) {
    return res.status(403).json({ error: 'Cannot approve a swap you are party to' });
  }
