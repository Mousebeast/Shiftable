...
const { setUserGroups, getUserWithGroups } = require('../db/users');
...
router.patch('/:id', requireManager, (req, res) => {
  const { name, color, hourly_rate } = req.body;
  if (!name || !color) return res.status(400).json({ error: 'name and color required' });
  const rate = hourly_rate !== undefined && hourly_rate !== '' ? Number(hourly_rate) : null;
  const group = updateGroup(Number(req.params.id), name, color, rate);
  if (!group) return res.status(404).json({ error: 'Group not found' });
...
// PATCH /api/groups/:id/members — manager+ (add/remove a single user from this group)
// Separate from PATCH /api/users/:id so admin accounts can be assigned to groups.
router.patch('/:id/members', requireManager, (req, res) => {
  const groupId = Number(req.params.id);
  const { userId, add } = req.body;
  if (!userId || add == null) {
    return res.status(400).json({ error: 'userId and add are required' });
  }
  if (!getGroupById(groupId)) return res.status(404).json({ error: 'Group not found' });
  const user = getUserWithGroups(Number(userId));
  if (!user) return res.status(404).json({ error: 'User not found' });
  const currentIds = user.groups.map(g => g.id);
  const newIds = add
    ? [...new Set([...currentIds, groupId])]
    : currentIds.filter(id => id !== groupId);
  setUserGroups(Number(userId), newIds);
  return res.json({ user: getUserWithGroups(Number(userId)) });
});
