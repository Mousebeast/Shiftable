...
describe('PATCH /api/admin/users/:id/role', () => {
  ...
  // These two previously asserted that admin was not a valid target role and
  // that admin accounts could never be altered. That combination made the
  // installer's admin account permanent and unique — it could not be joined,
  // demoted, deactivated, or deleted, so an owner whose tech-savvy installer
  // walked away had no route back in short of editing SQLite. Deliberate
  // behaviour change; the guard is now "never the last admin" rather than
  // "never an admin".
  ...
  it('rejects a role outside staff/manager/admin', async () => {
    const res = await request(app)
      .patch(`/api/admin/users/${staff.id}/role`)
      .set('Cookie', `token=${adminToken(admin.id)}`)
      .send({ role: 'superuser' });
  ...
  it('demotes an admin when another active admin remains', async () => {
    ...
    const res = await request(app)
      ...
      .send({ role: 'manager' });
    expect(res.status).toBe(200);
    const row = db.prepare('SELECT role FROM users WHERE id = ?').get(otherAdmin.id);
    expect(row.role).toBe('manager');
  });

  it('refuses to demote the last remaining admin', async () => {
    // `admin` is the only admin, and self-changes are blocked, so this is
    // exercised via a second admin demoting the original — after which the
    // actor would be the only one left.
    const otherAdmin = seedUser(db, { email: 'admin3@test.com', role: 'admin' });
    // Remove the second admin first so only one remains.
    db.prepare("UPDATE users SET role = 'manager' WHERE id = ?").run(otherAdmin.id);
    const res = await request(app)
      .patch(`/api/admin/users/${admin.id}/role`)
      .set('Cookie', `token=${adminToken(otherAdmin.id)}`)
      .send({ role: 'manager' });
    expect(res.status).toBe(409);
    const row = db.prepare('SELECT role FROM users WHERE id = ?').get(admin.id);
    expect(row.role).toBe('admin');
  });

  it('does not count a deactivated admin toward the last-admin guard', async () => {
    // A deactivated admin cannot log in, so it must not satisfy "an admin
    // still exists" — otherwise the last usable account can be demoted away.
    const ghost = seedUser(db, { email: 'ghost@test.com', role: 'admin' });
    db.prepare('UPDATE users SET is_active = 0 WHERE id = ?').run(ghost.id);
    const actor = seedUser(db, { email: 'admin4@test.com', role: 'admin' });
    db.prepare("UPDATE users SET role = 'manager' WHERE id = ?").run(actor.id);
    const res = await request(app)
      .patch(`/api/admin/users/${admin.id}/role`)
  ...
  it('still refuses to change your own role', async () => {
    const res = await request(app)
      .patch(`/api/admin/users/${admin.id}/role`)
      .set('Cookie', `token=${adminToken(admin.id)}`)
      .send({ role: 'manager' });
