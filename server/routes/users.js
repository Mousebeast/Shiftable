...
router.post('/', requireManager, (req, res) => {
  const { name, phone, role, minHours = 0, maxHours = 40, groupIds = [], minShifts = null, maxShifts = null } = req.body;
  ...
  sendWelcomeEmail(user, claimUrl).catch(() => {});
  sendClaimSms(user, claimUrl).catch(() => {});
...
router.patch('/:id', requireManager, (req, res) => {
  const { name, phone, role, minHours, maxHours, groupIds = [], minShifts, maxShifts } = req.body;
  const email = req.body.email?.trim().toLowerCase();
  if (!name || !email || !role) {
    return res.status(400).json({ error: 'name, email, and role are required' });
  }
  ...
  if (!target) return res.status(404).json({ error: 'User not found' });
  if (target.role === 'admin') {
    return res.status(403).json({ error: 'Cannot edit an admin account' });
  }
  ...
  const minS = minShifts !== undefined && minShifts !== '' ? Number(minShifts) : (minShifts === '' ? null : minShifts);
  const maxS = maxShifts !== undefined && maxShifts !== '' ? Number(maxShifts) : (maxShifts === '' ? null : maxShifts);
  if (minS != null && maxS != null && minS > maxS) {
    return res.status(400).json({ error: 'minShifts must be <= maxShifts' });
  }
