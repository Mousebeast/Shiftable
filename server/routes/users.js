...
router.post('/', requireManager, (req, res) => {
  const { name, phone, role, minHours = 0, maxHours = 40, groupIds = [] } = req.body;
...
router.patch('/:id', requireManager, (req, res) => {
  const { name, phone, role, minHours, maxHours, groupIds = [] } = req.body;
  const email = req.body.email?.trim().toLowerCase();
  if (!name || !email || !role) {
    return res.status(400).json({ error: 'name, email, and role are required' });
  }
  ...
  const user = updateUser(targetId, { name, email, phone, role, minHours, maxHours });
