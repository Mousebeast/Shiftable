...
router.post('/', requireManager, (req, res) => {
  const { name, email, phone, role, minHours = 0, maxHours = 40, groupIds = [] } = req.body;
  if (!name || !email || !role) {
  ...
  let user;
  try {
    user = createUser(name, email, role, minHours, maxHours, undefined, phone);
  } catch (err) {
...
router.patch('/:id', requireManager, (req, res) => {
  // Fix 4: default hours to undefined so DB helper can merge with existing values
  const { name, email, phone, role, minHours, maxHours, groupIds = [] } = req.body;
  if (!name || !email || !role) {
    return res.status(400).json({ error: 'name, email, and role are required' });
  }
  ...
  const user = updateUser(Number(req.params.id), { name, email, phone, role, minHours, maxHours });
