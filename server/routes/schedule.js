...
router.post('/copy', requireManager, (req, res) => {
  ...
  if (new Date(sourceWeek + 'T00:00:00Z').getUTCDay() !== 1 ||
      new Date(targetWeek + 'T00:00:00Z').getUTCDay() !== 1) {
    return res.status(400).json({ error: 'Both weeks must be Mondays' });
  }
  try {
...
router.post('/templates/:id/apply', requireManager, (req, res) => {
  ...
  if (new Date(targetWeek + 'T00:00:00Z').getUTCDay() !== 1) {
    return res.status(400).json({ error: 'targetWeek must be a Monday' });
  }
  try {
