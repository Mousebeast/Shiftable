...
// The client calls recheck() after every generate and replaces data.warnings
// wholesale with validateScheduleShifts' output. Any warning type the scheduler
// alone emits is therefore discarded before the manager can read it — it renders
// for one frame and vanishes. These assert the two produce the same *types*,
// which is what a per-type field comparison misses: a type emitted by only one
// path is not "single-source and safe", it is dropped.
describe('neither path emits a warning type the other drops', () => {
  function warningTypes(draftId) {
    const generated = regenerate(draftId);
    const validated = validateScheduleShifts(draftId, db).warnings;
    const types = ws => [...new Set(ws.map(w => w.type || 'coverage_gap'))].sort();
    return { generated: types(generated), validated: types(validated) };
  }

  test('min_shifts_unmet survives recheck', () => {
    // Emily's real case: a minimum set, nothing assigned, so the only signal that
    // she is unscheduled is this warning.
    const emily = seedUser(db, { name: 'Emily', email: 'emily@test.com', role: 'staff' });
    db.prepare('UPDATE users SET min_shifts_per_week = 2 WHERE id = ?').run(emily.id);

    const draft = createFreshDraftSchedule(WEEK, manager.id, db);
    const { generated, validated } = warningTypes(draft.id);

    expect(generated).toContain('min_shifts_unmet');
    expect(validated).toContain('min_shifts_unmet');
    expect(generated).toEqual(validated);
  });

  test('and names the person, who by definition may have no shift to read it from', () => {
    const emily = seedUser(db, { name: 'Emily Carter', email: 'emily@test.com', role: 'staff' });
    db.prepare('UPDATE users SET min_shifts_per_week = 2 WHERE id = ?').run(emily.id);

    const draft = createFreshDraftSchedule(WEEK, manager.id, db);
    regenerate(draft.id);

    for (const source of [regenerate(draft.id), validateScheduleShifts(draft.id, db).warnings]) {
      const w = source.find(x => x.type === 'min_shifts_unmet' && x.user_id === emily.id);
      expect(w).toMatchObject({ user_name: 'Emily Carter', assigned: 0, needed: 2 });
    }
  });

  test('the count is live, not frozen at generate time', () => {
    // Deliberately in no group, so the scheduler cannot assign her and the only
    // shift she ends up with is the one added by hand below. That isolates the
    // property under test: does the count move without a regenerate?
    const emily = seedUser(db, { name: 'Emily', email: 'emily@test.com', role: 'staff' });
    db.prepare('UPDATE users SET min_shifts_per_week = 2 WHERE id = ?').run(emily.id);
    ...
    const before = validateScheduleShifts(draft.id, db).warnings
      .find(x => x.type === 'min_shifts_unmet' && x.user_id === emily.id);
    expect(before).toMatchObject({ assigned: 0, needed: 2 });

    // Hand-assign one shift. No regenerate — this is what the manager does.
    insertOverrideShift(draft.id, {
      userId: emily.id, groupId: host.id, templateId: hostTemplate.id,
      startTime: '16:00', hours: 5, date: '2026-07-28',
    }, db);

    const after = validateScheduleShifts(draft.id, db).warnings
      .find(x => x.type === 'min_shifts_unmet' && x.user_id === emily.id);
    expect(after).toMatchObject({ assigned: 1, needed: 2 });
  });

  test('no type appears in one path and not the other', () => {
    const emily = seedUser(db, { name: 'Emily', email: 'emily@test.com', role: 'staff' });
    db.prepare('UPDATE users SET min_shifts_per_week = 2 WHERE id = ?').run(emily.id);
    ...
    expect(generated).toEqual(validated);
    // Guard against passing because both are empty.
