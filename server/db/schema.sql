...
CREATE TABLE IF NOT EXISTS availability (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  day_of_week INTEGER NOT NULL, -- offset from the week start; 0 = first day of week
  start_time TEXT NOT NULL, -- HH:MM
  end_time TEXT NOT NULL,   -- HH:MM
  is_blocked TINYINT NOT NULL DEFAULT 0,
  effective_from TEXT NOT NULL, -- ISO date, must fall on the configured week start
...
-- schedule_template_shifts: shifts stored as day_of_week (offset from week start,
-- 0 = first day of the week — see WEEK_START in .env), not dates
