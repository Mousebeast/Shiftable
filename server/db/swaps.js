...
// The status update runs first as a guard: if the swap has already been resolved
// (concurrent request or double-tap), changes===0 and the shift transfer is skipped.
function resolveAndTransfer(swapId, status, db = _db) {
  db.transaction(() => {
    const swap = db.prepare(
      `SELECT original_shift_id, claimer_id FROM shift_swaps WHERE id = ?`
    ).get(swapId);
    if (!swap || !swap.claimer_id) return;
    const result = db.prepare(
      `UPDATE shift_swaps SET status = ?, resolved_at = unixepoch()
       WHERE id = ? AND status IN ('claimed', 'pending_manager')`
    ).run(status, swapId);
...
function countResolvedSwapsSince(publishedScheduleId, since, db = _db) {
  const row = db.prepare(
    `SELECT COUNT(*) AS c FROM shift_swaps sw
     JOIN schedule_shifts ss ON ss.id = sw.original_shift_id
     WHERE ss.schedule_id = ?
       AND sw.status IN ('auto_approved', 'manager_approved')
       AND sw.resolved_at > ?`
  ).get(publishedScheduleId, since);
  return row.c;
}
...
module.exports = {
  ...
  getPendingSwaps, countResolvedSwapsSince,
};
