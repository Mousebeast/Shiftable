...
function getAllGroups(db = _db) {
  return db.prepare(
    `SELECT id, name, color, hourly_rate, priority, created_at FROM groups ORDER BY priority ASC, name`
  ).all();
}

function getGroupById(id, db = _db) {
  return db.prepare(
    `SELECT id, name, color, hourly_rate, priority, created_at FROM groups WHERE id = ?`
  ).get(id);
}

function createGroup(name, color, db = _db) {
  return db.prepare(
    `INSERT INTO groups (name, color) VALUES (?, ?) RETURNING id, name, color, hourly_rate, priority`
  ).get(name, color);
}

function updateGroup(id, name, color, hourlyRate, db = _db) {
  return db.prepare(
    `UPDATE groups SET name = ?, color = ?, hourly_rate = ? WHERE id = ? RETURNING id, name, color, hourly_rate, priority`
  ).get(name, color, hourlyRate ?? null, id);
}
...
function getAllGroupsWithDetail(db = _db) {
  const groups = db.prepare(
    `SELECT id, name, color, hourly_rate, priority FROM groups ORDER BY priority ASC, name`
  ).all();
  const templates = db.prepare(
    `SELECT id, group_id, name, start_time, hours FROM shift_templates ORDER BY group_id, start_time`
  ).all();
  const coverage = db.prepare(
    `SELECT cr.id, cr.group_id, cr.day_of_week, cr.shift_template_id, cr.min_staff,
            st.name AS template_name, st.start_time
     FROM coverage_rules cr
     JOIN shift_templates st ON st.id = cr.shift_template_id
