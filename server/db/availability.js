...
const { localWeekStartOf } = require('../services/dates');
...
function getAllStaffAvailability(db = _db) {
  ...
  // Only rows already in force. Without the date bound this returned the row
  // with the newest effective_from outright, including future-dated ones — so
  // a manager editing a cell would save successfully and then see an approved
  // staff request dated weeks out render in its place. Mirrors the bound in
  // getSchedulerInputs and checkShiftViolations; the three must agree.
  const thisWeek = localWeekStartOf(new Date());
