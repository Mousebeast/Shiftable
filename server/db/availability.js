...
function getAvailabilityForUser(userId, db = _db) {
  return db
    .prepare(
      `SELECT id, day_of_week, start_time, end_time, effective_from, status, created_at
       FROM availability
       WHERE user_id = ?
...
function getCurrentApprovedPattern(userId, db = _db) {
  return db
    .prepare(
      `SELECT day_of_week, start_time, end_time, effective_from
       FROM availability
       WHERE user_id = ? AND status = 'approved'
       GROUP BY day_of_week
       HAVING effective_from = MAX(effective_from)
...
function createAvailabilityEntries(userId, entries, db = _db) {
  const insert = db.prepare(
    `INSERT INTO availability (user_id, day_of_week, start_time, end_time, effective_from, status)
     VALUES (@userId, @day_of_week, @start_time, @end_time, @effective_from, 'pending')`
  );
...
function getPendingAvailability(db = _db) {
  return db
    .prepare(
      `SELECT a.id, a.user_id, a.day_of_week, a.start_time, a.end_time, a.effective_from,
...
function getAllStaffAvailability(db = _db) {
  ...
  const allAvail = db.prepare(
    `SELECT user_id, day_of_week, start_time, end_time
     ...
     ORDER BY user_id, day_of_week, effective_from DESC`
  ).all();
  const availMap = {};
...
function upsertManagerAvailability(userId, dayOfWeek, startTime, endTime, managerId, effectiveFrom, db = _db) {
  db.prepare(
  ...
  ).run(userId, dayOfWeek, startTime, endTime, effectiveFrom, managerId);
}
