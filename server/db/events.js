...
function upsertEventCoverage(weekStart, date, groupId, templateId, requiredStaff, db = _db) {
  db.prepare(
    `INSERT INTO week_event_coverage (week_start_date, date, group_id, shift_template_id, required_staff)
     VALUES (?, ?, ?, ?, ?)
     ON CONFLICT(week_start_date, date, group_id, shift_template_id) DO UPDATE SET
       required_staff = excluded.required_staff`
  ).run(weekStart, date, groupId, templateId, requiredStaff);
}

function deleteEventCoverageRow(weekStart, date, groupId, templateId, db = _db) {
  db.prepare(
    `DELETE FROM week_event_coverage WHERE week_start_date = ? AND date = ? AND group_id = ? AND shift_template_id = ?`
  ).run(weekStart, date, groupId, templateId);
}
