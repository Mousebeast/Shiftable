...
function createUser(name, email, role, minHours, maxHours, db = _db, phone = null) {
  return db
    .prepare(
      `INSERT INTO users (name, email, phone, role, min_hours_per_week, max_hours_per_week)
       VALUES (?, ?, ?, ?, ?, ?) RETURNING id, name, email, phone, role, min_hours_per_week, max_hours_per_week, is_active`
    )
    .get(name, email, phone || null, role, minHours, maxHours);
}

function updateUser(id, { name, email, phone, role, minHours, maxHours }, db = _db) {
  const existing = db.prepare(
    `SELECT min_hours_per_week, max_hours_per_week, phone FROM users WHERE id = ?`
  ).get(id);
  ...
  return db
    .prepare(
      `UPDATE users SET name = ?, email = ?, phone = ?, role = ?, min_hours_per_week = ?, max_hours_per_week = ?
       WHERE id = ?
       RETURNING id, name, email, phone, role, min_hours_per_week, max_hours_per_week, is_active`
    )
    .get(name, email, finalPhone, role, finalMin, finalMax, id);
}
