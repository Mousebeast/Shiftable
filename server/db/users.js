...
// Deactivation only ever touched is_active, so everything else about the user —
// groups, PIN hash, hour and shift limits, priority position, fixed schedules —
// survived it and comes back untouched. The one thing that does not is any open
// swap the route cancelled on the way out; those are gone deliberately.
function reactivateUser(id, db = _db) {
 ...
 *
 * Also drops any fixed_schedules row whose shift template belongs to a group
 * the user is no longer in. A fixed_schedules row stores a shift_template_id,
 * and templates belong to groups — so the row silently encodes membership.
 * Leaving one behind means the scheduler keeps assigning the old group's
 * shifts: Phase 1 locks fixed schedules before the fill phase and never checks
 * user_groups, so the stale row wins over every other constraint.
 *
 * Destructive by design: removing someone from a group removes the fixed
 * shifts that only made sense as a member of it. Re-adding the group later
...
function setUserGroups(userId, groupIds, db = _db) {
  ...
  db.transaction((uid, gids) => {
    ...
    // Runs after the new memberships are in place, so it prunes against the
    // final set rather than the old one.
    pruneFixed.run(uid, uid);
  })(userId, groupIds);
}

/**
 * Fixed schedules that would be removed if the user's groups became `groupIds`.
 * Lets the UI warn before the destructive part of setUserGroups happens.
 */
function fixedSchedulesOutsideGroups(userId, groupIds, db = _db) {
  if (!Array.isArray(groupIds)) return [];
  const base =
    `SELECT fs.day_of_week, st.name AS template_name, g.name AS group_name
       FROM fixed_schedules fs
       JOIN shift_templates st ON st.id = fs.shift_template_id
       JOIN groups g ON g.id = st.group_id
      WHERE fs.user_id = ?`;
  // Empty group list is its own branch on purpose. `x NOT IN (NULL)` evaluates
  // to unknown rather than true in SQL, so folding this into the NOT IN below
  // would silently report that nothing is at risk in exactly the case where
  // everything is.
  if (groupIds.length === 0) {
    return db.prepare(`${base} ORDER BY fs.day_of_week`).all(userId);
  }
  const placeholders = groupIds.map(() => '?').join(',');
  return db.prepare(
    `${base} AND st.group_id NOT IN (${placeholders}) ORDER BY fs.day_of_week`
  ).all(userId, ...groupIds);
}

// Everyone who can act on the Approvals queue. Admins are included, not just
// managers: the role model gives both approve rights on swaps, time-off and
// availability, and on a small install the admin is often the only person who
// looks. `excludeUserId` keeps a manager filing their own request from being
// notified about it.
//
// Deactivated users are excluded — authenticate 401s them, so a notification
// they can never open is noise in the table and a wasted push.
function getApproverIds(excludeUserId = null, db = _db) {
  return db.prepare(
    `SELECT id FROM users
     WHERE is_active = 1 AND role IN ('manager', 'admin') AND id != ?`
  ).all(excludeUserId ?? -1).map(r => r.id);
}
