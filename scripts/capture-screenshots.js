...
function seedDB() {
  ...
  // Groups
  const gSrv  = db.prepare("INSERT INTO groups (name,color) VALUES ('Servers','#3b82f6')").run().lastInsertRowid;
  const gBar  = db.prepare("INSERT INTO groups (name,color) VALUES ('Bartenders','#8b5cf6')").run().lastInsertRowid;
  const gHost = db.prepare("INSERT INTO groups (name,color) VALUES ('Hosts','#10b981')").run().lastInsertRowid;
...
async function main() {
  ...
  try {
    ...
    // Full week grid
    await mPage.goto(`${BASE}/schedule/week`, { waitUntil: 'networkidle' });
    ...
    await dPage.goto(`${BASE}/schedule/week`, { waitUntil: 'networkidle' });
