...
export function useScheduleBuilder(weekStart) {
  ...
  const updateShift = useCallback(async (shiftId, { groupId, templateId, startTime, hours }) => {
    const r = await fetch(`/api/schedule/shifts/${shiftId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ groupId, templateId, startTime, hours }),
    });
    const d = await r.json();
    if (r.ok) {
  ...
  const addShift = useCallback(async (scheduleId, { userId, date, groupId, templateId, startTime, hours }) => {
    const r = await fetch(`/api/schedule/${scheduleId}/shifts`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ userId, date, groupId, templateId, startTime, hours }),
    });
    const d = await r.json();
    if (r.ok) {
  ...
  const closeDay = useCallback(async (scheduleId, date) => {
    ...
    if (r.ok) {
      ...
      setViolations(prev => prev.filter(v => newShiftIds.has(v.shiftId)));
      setHasEdits(true);
    }
    return d;
  }, []);
  ...
  const saveEventCoverage = useCallback(async (weekStart, date, groupId, templateId, requiredStaff) => {
    ...
    if (r.ok) {
      setData(prev => {
        const filtered = prev.eventCoverage.filter(ec => ec.date !== date);
        return {
          ...prev,
  ...
  return {
    ...data,
    loading, generating, publishing, republishing, forking, discarding, error,
    violations, hasEdits,
