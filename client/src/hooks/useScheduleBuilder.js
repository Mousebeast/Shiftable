...
export function useScheduleBuilder(weekStart) {
  ...
  // mode: 'full' runs the whole scheduler; 'fixed-only' stops after fixed shifts
  // and manual overrides, leaving the rest for the manager to fill in.
  const generate = useCallback(async ({ force = false, mode = 'full' } = {}) => {
    setGenerating(true);
    setError(null);
    try {
      const r = await fetch('/api/schedule/generate', {
        ...
        credentials: 'include',
        body: JSON.stringify({ week: weekStart, force, mode }),
      });
    ...
    // recheck is deliberately omitted. It is declared ~250 lines below this
    // point, and a dependency array is evaluated during render — listing it here
    // throws "Cannot access 'recheck' before initialization" on every render.
    // Omitting it is safe rather than merely convenient: recheck is
    // useCallback(..., []), so its identity never changes for the life of the
    // hook, and it reads scheduleStatusRef.current plus setState functions only.
    // There is no stale closure to capture.
    ...
    try {
      ...
      if (r.ok) {
        setData(prev => ({
          ...
          hasDraft: false,
          liveSchedule: null,
          // Cleared alongside violations, not instead of it. `violations` is its
          // own useState while `warnings` lives inside `data`, so resetting one
          // does nothing for the other — and the Schedule tab badge sums BOTH.
          // Miss this and the badge keeps a count for a schedule that is gone.
          warnings: [],
        }));
        setHasEdits(false);
        setViolations([]);
    ...
    try {
      ...
      if (r.ok) {
        setData(prev => ({
          ...
          hasDraft: true,
          liveSchedule: d.liveSchedule || null,
          // See the note in republish. The auto-validate effect refills these a
          // moment later here, but clearing is still correct: it avoids showing
          // the previous schedule's count during the round trip.
          warnings: [],
        }));
        setViolations([]);
        setHasEdits(false);
    ...
    try {
      ...
      if (r.ok) {
        setData(prev => ({
          ...
          hasDraft: false,
          liveSchedule: d.liveSchedule || null,
          // The important one. After a discard `schedule` is null, so the
          // auto-validate effect (keyed on schedule.id) never re-runs and
          // nothing else will ever replace this array. Leave it and the tab
          // badge shows a warning count on the empty-state screen, where there
          // is no grid, no tray, and no way to clear it.
          warnings: [],
        }));
        setViolations([]);
        setHasEdits(false);
