...
export default function Layout() {
  ...
  // Always call subscribe on mount — pushManager.subscribe() returns the existing
  // subscription silently if one exists, ensuring it's always saved to the server.
  useEffect(() => {
    // Temporary diagnostic — remove after push is confirmed working
    fetch(`/api/health?ps=${isSupported ? 1 : 0}&perm=${Notification?.permission || 'na'}`, { credentials: 'include' });
    if (isSupported) subscribe();
  }, [isSupported]);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-950">
      <header className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between sticky top-0 z-20 relative after:content-[''] after:absolute after:top-full after:left-0 after:right-0 after:h-2 after:bg-gray-950 after:pointer-events-none">
