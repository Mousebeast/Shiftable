...
export default function GroupManagement() {
  ...
  const { users, updateUser } = useStaff();
  ...
  async function handleUpdateMember(targetUser, groupId, adding) {
    const currentGroupIds = (targetUser.groups || []).map(g => g.id);
    ...
    return await updateUser(targetUser.id, {
      ...
      minHours: targetUser.min_hours_per_week,
      maxHours: targetUser.max_hours_per_week,
                ...
                {editGroupId === g.id ? (
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => { setEditGroupId(g.id); setEditForm({ name: g.name, color: g.color }); }}
