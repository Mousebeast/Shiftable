...
function GroupDetailPanel({ groupId, allUsers, onUpdateMember }) {
  ...
  // Collapsed by default: this list is one row per active staff member, so on a
  // full roster it pushes Shift Templates — the section actually edited here —
  // off the screen.
  ...
  // Counted from the active list only, so the collapsed header's "N of M"
  // cannot read 5 of 4 when a former member has since been deactivated.
  const memberIds = new Set(
    activeUsers.filter(u => u.groups && u.groups.some(g => g.id === groupId)).map(u => u.id)
  );
    ...
    <div className="bg-gray-900 rounded-xl p-4 space-y-5 text-sm">
      ...
      <section className="space-y-2">
        ...
        >
          ...
          {/* The count is what makes collapsing an improvement rather than a
              click tax — you learn the membership without opening anything. */}
          <span className="text-xs text-gray-500">
            {memberIds.size} of {activeUsers.length}
          </span>
          <span className="text-xs text-gray-500 ml-auto">{membersOpen ? '▲' : '▼'}</span>
        </button>
...
export default function GroupManagement() {
  ...
  const { users, updateMembership } = useStaff();
