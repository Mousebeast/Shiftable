...
export default function SwapShift() {
  ...
  const load = useCallback(() => {
    Promise.all([
      fetch('/api/swaps', { credentials: 'include' }).then((r) => r.json()),
      fetch('/api/schedule/next', { credentials: 'include' }).then((r) => r.json()),
    ]).then(([swapData]) => {
  ...
  async function handleCancel(swapId) {
    await fetch(`/api/swaps/${swapId}`, { method: 'DELETE', credentials: 'include' });
    load();
  }
  ...
  async function loadUpcomingShifts() {
    ...
    const res = await fetch(`/api/schedule?week=${weekStart}`, { credentials: 'include' });
    const data = await res.json();
    const shifts = (data.shifts || []).filter((s) => s.user_id === user.id && s.date >= today);
