...
function TimeOffCard({ item, onApprove, onDeny }) {
    ...
    <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-3">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 py-2 text-xs font-medium bg-green-700 hover:bg-green-600 text-white rounded-lg transition-colors"
        >
          Approve
        </button>
        <button
        ...
        <button
          ...
          className="flex-1 py-2 text-xs font-medium bg-red-900 hover:bg-red-800 text-white rounded-lg transition-colors"
        >
          Deny
        </button>
      </div>
...
function AvailabilityCard({ group, onApprove, onDeny }) {
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-3">
      <div>
        ...
        {group.rows
          ...
          .map(row => (
            <p key={row.id} className="text-xs text-gray-300">
              {DAY_NAMES[row.day_of_week]}: {formatTime(row.start_time)}–{formatTime(row.end_time)}
            </p>
          ))}
      </div>
      <div className="flex gap-2">
...
function SwapCard({ swap, onApprove, onDeny }) {
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-3">
      <div>
