...
import { Lock, CheckCircle2, ChevronDown, RefreshCw, Printer, Eye, StickyNote } from 'lucide-react';
...
function EditableDraftGrid({ weekStart, shifts, violations, warnings = [], timeOffDates = [], onUpdateShift, onRemoveShift, onAddShift, scheduleId, readOnly, closedDays, onCloseDay, onOpenDay, events = {}, eventCoverage = [], onAddEventTitle, onDeleteEventTitle, onSaveEventCoverage, onRemoveEventCoverage, viewers = [] }) {
  ...
  const personalViolationTypes = new Set(['availability', 'not_available', 'hours_cap', 'fixed_conflict', 'min_hours']);
            ...
            <tr className="bg-gray-950">
              ...
              {days.map(d => {
                ...
                return (
                  <th
                    key={d}
                    onClick={readOnly ? (e) => { const rect = e.currentTarget.getBoundingClientRect(); setOpenShiftPopover({ date: d, x: rect.left, y: rect.bottom + 4 }); } : (e) => openDayPopover(e, d)}
                    className={`border border-gray-700 px-2 py-2 font-medium min-w-[80px] whitespace-nowrap text-center cursor-pointer ${
                      readOnly ? 'hover:bg-green-900/20' : 'hover:bg-blue-900/20'
    ...
    <div className="mx-4 mt-2 mb-1 border border-gray-700 rounded-lg overflow-hidden">
      ...
      {expanded && total > 0 && (
        <div className="bg-gray-900/50 px-3 py-2 space-y-3">
          {(warnings.length > 0 || coverageViolations.length > 0) && (
            <div>
...
export default function ScheduleBuilder() {
  ...
  const scheduleContextText = loading ? ''
    : !schedule ? 'No schedule for this week — press Generate to build one.'
    ...
    : isPublished ? 'Published — tap Start Editing to make changes.'
    : '';
