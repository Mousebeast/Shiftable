...
import { Lock, CheckCircle2, ChevronDown, RefreshCw, Printer } from 'lucide-react';
...
function EditableDraftGrid({ weekStart, shifts, violations, warnings = [], timeOffDates = [], onUpdateShift, onRemoveShift, onAddShift, scheduleId, readOnly, closedDays, onCloseDay, onOpenDay, events = {}, eventCoverage = [], onAddEventTitle, onDeleteEventTitle, onSaveEventCoverage, onRemoveEventCoverage }) {
  ...
  // Append ungrouped active staff as empty rows so managers can add override shifts
  refData.users
    .filter(u => u.groups.length === 0 && !usersMap[u.id])
  ...
  const personalViolationTypes = new Set(['availability', 'hours_cap', 'fixed_conflict']);
  const userViolationCounts = {};
  const userSeenTypes = {};
  violations.forEach(v => {
    if (!v.shiftId || !personalViolationTypes.has(v.type)) return;
    const s = shifts.find(sh => sh.id === v.shiftId);
    if (!s) return;
    if (!userSeenTypes[s.user_id]) userSeenTypes[s.user_id] = new Set();
    if (userSeenTypes[s.user_id].has(v.type)) return;
    userSeenTypes[s.user_id].add(v.type);
    userViolationCounts[s.user_id] = (userViolationCounts[s.user_id] || 0) + 1;
  });
  ...
  function closePopover() { setPopover(null); setSaving(false); setSelectedTemplateId(''); setSaveError(''); }
  ...
  return (
    <div>
            ...
            <tr className="bg-gray-950">
              ...
              {days.map(d => {
                ...
                return (
                  <th
                    key={d}
                    onClick={readOnly ? undefined : (e) => openDayPopover(e, d)}
                    className={`border border-gray-700 px-2 py-2 font-medium min-w-[80px] whitespace-nowrap text-center ${
                      readOnly ? '' : 'cursor-pointer hover:bg-blue-900/20'
                    } ${isClosed ? 'text-red-400' : 'text-gray-300'}`}
                  >
                    {new Date(d + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'numeric', day: 'numeric' })}
                    {isClosed && <span className="block text-[10px] font-normal">(Closed)</span>}
                    {!isClosed && coverageGapsByDate[d] > 0 && (
                      <span className="block text-[9px] font-bold text-amber-400">⚠{coverageGapsByDate[d]}</span>
                    )}
                  </th>
                );
              })}
              <th className="border border-gray-700 px-2 py-2 font-medium text-gray-300 min-w-[45px] text-center">
                Hrs
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              ...
              {days.map(date => {
                ...
                const dayCoverage = eventCoverage.find(ec => ec.date === date);
                return (
                  <td
                    key={date}
                    ...
                    <div className="overflow-hidden min-h-[18px]">
                      {dayTitles.map(t => (
                        <span key={t.id} className="inline-flex items-center gap-0.5 bg-purple-900/50 border border-purple-700/50 text-purple-200 text-[9px] px-1 py-0.5 rounded mr-0.5 mb-0.5 max-w-full">
                          <span className="truncate max-w-[56px]">{t.title}</span>
                          {!readOnly && (
                            <button
            ...
            {userRows.map(([userId, userName]) => (
              <tr key={userId} className="hover:bg-gray-700/30">
                ...
                {days.map(date => {
                  ...
                  const isTimeOff = !isClosed && cellShifts.length === 0 && timeOffSet.has(`${userId}|${date}`);
                  return (
                    <td
                      key={date}
                      onClick={readOnly ? undefined : (e) => openPopover(e, userId, userName, date)}
                      className={`relative border border-gray-700 px-1 py-1 align-top transition-colors ${
                    ...
                    >
                      ...
                      {isClosed ? (
                        ...
                        >
                          ...
                          <span className="hidden print:inline">{s.group_name} · {formatTime(s.start_time)}</span>
                        </div>
                      ) ) : isTimeOff ? (
      ...
      {popover && (
        <>
          ...
          <div
            ...
            style={{
              top: Math.min(popover.y, window.innerHeight - 240),
      ...
      {eventPopover && (() => {
        ...
        const dayTitles = events[date] || [];
        const existingCoverage = eventCoverage.find(ec => ec.date === date);
...
function EventCellPopover({ date, x, y, weekStart, dayTitles, existingCoverage, allGroups, allTemplates, onAddTitle, onDeleteTitle, onSaveCoverage, onRemoveCoverage, onClose }) {
  const [titleInput, setTitleInput] = useState('');
  const [selectedGroup, setSelectedGroup] = useState(existingCoverage?.group_id?.toString() || '');
  const [selectedTemplate, setSelectedTemplate] = useState(existingCoverage?.shift_template_id?.toString() || '');
  const [staffCount, setStaffCount] = useState(existingCoverage?.required_staff || 1);
  const [saving, setSaving] = useState(false);
  ...
  async function handleSave() {
    ...
    try {
      if (selectedGroup && selectedTemplate) {
        await onSaveCoverage?.(weekStart, date, Number(selectedGroup), Number(selectedTemplate), Number(staffCount));
      }
    } finally {
      setSaving(false);
      onClose();
  ...
  const canSave = dayTitles.length > 0 && (!selectedGroup || selectedTemplate);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <option value="">— No coverage required —</option>
        ...
        {existingCoverage && (
          <button
            onClick={async () => { await onRemoveCoverage?.(weekStart, date); setSelectedGroup(''); setSelectedTemplate(''); }}
            className="w-full mb-2 py-1 text-[11px] text-red-400 hover:text-red-300 border border-red-800/50 hover:border-red-700 rounded transition-colors"
          >
            Clear Coverage
          </button>
        )}
...
function ViolationsTray({ warnings = [], violations = [], shifts = [], onRecheck, onPrint }) {
  ...
  function shortWarning(w) {
    const group = groupNames[w.group_id] || 'Group';
    if (w.type === 'event_coverage_gap') {
      const dateLabel = new Date(w.date + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'numeric', day: 'numeric' });
      return `${group} ${dateLabel}: ${w.assigned}/${w.needed} for event`;
    }
    return `${group} ${dayNames[w.day_of_week]}: ${w.assigned}/${w.needed} staffed`;
  }

  function shortViolation(v) {
    const shift = shifts.find(s => s.id === v.shiftId);
    const firstName = shift ? shift.user_name.split(' ')[0] : '?';
    ...
    switch (v.type) {
      ...
      case 'coverage_gap': return `${shift?.group_name || 'Group'} ${day}: understaffed`;
      case 'fixed_conflict': return `${firstName} ${day}: overrides fixed`;
      default: return v.message;
    }
  }

  // Deduplicate violations by user+type so hours_cap shows once per person, not once per shift
  const seenKeys = new Set();
  const dedupedViolations = violations.filter(v => {
    const shift = shifts.find(s => s.id === v.shiftId);
    const key = `${shift?.user_id ?? '?'}|${v.type}`;
  ...
  return (
    <div className="mx-4 mt-2 mb-1 border border-gray-700 rounded-lg overflow-hidden">
      <div className={`flex items-center gap-2 px-3 py-2 ${total > 0 ? 'bg-amber-900/25 border-amber-800/50' : 'bg-gray-800/60'}`}>
        {total > 0 ? (
          <button
          ...
          >
            ...
            <ChevronDown size={11} className={`text-amber-500 transition-transform flex-shrink-0 ${expanded ? 'rotate-180' : ''}`} />
          </button>
        ) : (
      ...
      {expanded && total > 0 && (
        <div className="bg-gray-900/50 px-3 py-2 space-y-1">
          {warnings.map((w, i) => (
...
function RepublishModal({ violations = [], shifts = [], onConfirm, onCancel }) {
...
function CoverageGrid() {
  ...
  useEffect(() => {
    fetch('/api/groups/all-detail', { credentials: 'include' })
      .then(r => r.json())
  ...
  async function handleAddRule() {
    ...
    if (res.ok) {
      ...
      const enriched = { ...rule, template_name: tpl?.name || '' };
  ...
  return (
    <div>
      <div className="px-4 py-2 border-b border-gray-700">
        <p className="text-xs text-gray-500">Drag a group row to reorder — top fills first when staff belong to multiple groups. Click any day cell to manage coverage rules.</p>
      </div>
              ...
              <div className="space-y-1.5 mb-3">
                {popoverRules.map(r => (
                  <div key={r.id} className="flex items-center justify-between gap-2">
                    <span className="text-[11px] text-gray-300">{r.template_name}: {r.min_staff}</span>
                    <button
...
function AvailabilityGrid() {
  ...
  function openPopover(e, userId, dayOfWeek, avail) {
    const rect = e.currentTarget.getBoundingClientRect();
    const isAllDay = avail?.start_time === '00:00' && avail?.end_time === '23:59';
    setPopover({
      userId,
      dayOfWeek,
      x: rect.left,
  ...
  async function handleSave() {
    ...
    setSaving(true);
    setSaveError('');
    const startTime = popover.isAllDay ? '00:00' : popover.startTime;
    const endTime = popover.isAllDay ? '23:59' : popover.endTime;
    const effectiveFrom = getNextMonday();
    const res = await fetch(`/api/availability/manager/${popover.userId}/${popover.dayOfWeek}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ startTime, endTime, effectiveFrom }),
    });
    if (res.ok) {
      setStaff(prev => prev.map(s => {
        if (s.id !== popover.userId) return s;
        const filtered = s.availability.filter(a => a.day_of_week !== popover.dayOfWeek);
        return { ...s, availability: [...filtered, { day_of_week: popover.dayOfWeek, start_time: startTime, end_time: endTime }] };
      }));
      setSaving(false);
      closePopover();
    } else {
  ...
  return (
    <div>
      <div className="px-4 py-2 border-b border-gray-700">
        <p className="text-xs text-gray-500">Click any cell to set or clear availability. Changes take effect from next Monday.</p>
      </div>
      <div className="overflow-x-auto">
        <table className="text-xs border-collapse min-w-full">
          <thead>
                    ...
                    >
                      {avail ? (
                        <span className="text-green-400 text-[10px] leading-tight">
                          {isAllDay ? 'All day' : <>{toDisplayTime(avail.start_time)}<br />{toDisplayTime(avail.end_time)}</>}
                        </span>
                      ) : (
                        <span className="text-gray-600 text-[10px]">—</span>
                      )}
      ...
      {popover && (
        <>
          ...
          >
            ...
            <button
              onClick={() => setPopover(p => ({ ...p, isAllDay: !p.isAllDay }))}
...
export default function ScheduleBuilder() {
  ...
  const {
    schedule, shifts, warnings, loading, generating, publishing,
    republishing, forking, discarding, error, violations, hasEdits, hasDraft, liveSchedule,
    closedDays, timeOffDates, events, eventCoverage,
    generate, publish, republish, fork, discardDraft,
    updateShift, removeShift, addShift,
    closeDay, openDay, recheck,
    addEventTitle, deleteEventTitle, saveEventCoverage, removeEventCoverage,
  ...
  async function handleDiscardDraft() {
    if (!schedule) return;
    if (!window.confirm('Discard this working draft? The schedule will revert to the current published version.')) return;
  ...
  return (
    <div className="max-w-full">
      <div className="sticky top-[57px] z-10 print:hidden">
        ...
        {tab === 'schedule' && (
          <div className="flex items-center justify-between px-4 py-2 bg-gray-800 border-b border-gray-700 gap-2">
            <div className="flex items-center gap-1 min-w-0">
              <button onClick={prevWeek} className="px-2 py-1 text-gray-400 hover:text-gray-200 text-base leading-none">‹</button>
              <span className={`text-sm font-medium whitespace-nowrap ${weekStart === getMondayOf(new Date()) ? 'text-blue-400' : 'text-gray-100'}`}>Week of {formatDate(weekStart)}</span>
              <button onClick={nextWeek} className="px-2 py-1 text-gray-400 hover:text-gray-200 text-base leading-none">›</button>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              {/* Re-Generate: only for fork drafts */}
              {isForkDraft && (
                <button
              ...
              {/* Discard Draft: only for fork drafts */}
              {isForkDraft && (
                <button
                  onClick={handleDiscardDraft}
                  disabled={generating || forking || discarding || republishing}
              ...
              {/* Published state: badge + Start Editing */}
              {isPublished && (
                <>
              ...
              {/* Fresh draft: Regenerate + Publish */}
              {isFreshDraft && (
                <>
                  <button
              ...
              {!schedule && (
                <button
                  onClick={handleGenerate}
                  disabled={generating || loading}
                  className="px-3 py-1.5 text-xs font-medium bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded transition-colors"
                >
                  {generating ? 'Generating…' : 'Generate'}
                </button>
              )}

              {/* Re-Publish: only for fork drafts */}
              {isForkDraft && (
                <button
                  onClick={handleRepublish}
                  disabled={republishing || discarding}
                  className="px-3 py-1.5 text-xs font-medium bg-green-600 hover:bg-green-500 disabled:opacity-50 text-white rounded transition-colors"
                >
      ...
      {tab === 'schedule' && (
        <>
          ...
          {isForkDraft && (
            <div className="mx-4 mt-3 p-3 bg-blue-900/30 border border-blue-700 rounded-lg flex items-center gap-2 print:hidden">
              <span className="text-xs text-blue-300 font-medium">Draft in Progress</span>
              <span className="text-xs text-blue-400">— staff are seeing the published version. Edit freely, then Re-Publish when ready.</span>
            </div>
          )}
          {loading ? (
            ...
            <>
              <div className="hidden print:block px-4 pt-3 pb-1">
                <p className="text-sm font-semibold text-black">Schedule — Week of {formatDate(weekStart)}</p>
              </div>
              <div className="mt-2">
                <EditableDraftGrid
                  weekStart={weekStart}
              ...
              <div className="print:hidden">
                <ViolationsTray warnings={warnings} violations={violations} shifts={shifts} onRecheck={() => recheck(schedule?.id)} onPrint={() => window.print()} />
              </div>
            </>
          ) : (
            <div className="p-8 text-center space-y-1">
              <p className="text-gray-400 text-sm">No schedule for this week.</p>
              <p className="text-gray-500 text-xs">Click Generate to build a draft.</p>
            </div>
          )}
        </>
      )}

      {tab === 'coverage' && <CoverageGrid />}
