    ...
    <div>
      <div className="overflow-x-auto">
        <table className="text-xs border-collapse min-w-full">
          ...
          <tfoot>
            <tr>
              <td className="sticky left-0 bg-gray-800 border border-gray-700 px-2 py-1.5 text-[10px] font-medium text-gray-500 z-10 whitespace-nowrap print:hidden">
                Est. Labor
              </td>
              {days.map(d => (
                <td key={d} className="border border-gray-700 px-2 py-1.5 text-center text-[10px] text-gray-400 bg-gray-800 print:hidden">
                  {fmtLabor(laborByDay[d] || 0)}
                </td>
              ))}
              <td className="border border-gray-700 px-2 py-1.5 text-center text-[10px] font-semibold text-blue-300 bg-blue-950 print:hidden">
                {fmtLabor(laborTotal)}
              </td>
            </tr>
          </tfoot>
...
function ViolationsTray({ warnings = [], violations = [], shifts = [], onRecheck, onPrint, onSaveTemplate }) {
  ...
  function shortWarning(w) {
    if (w.type === 'min_shifts_unmet') {
      const name = shifts.find(s => s.user_id === w.user_id)?.user_name?.split(' ')[0] ?? 'Staff';
            ...
            <thead>
              <tr className="bg-gray-950">
                ...
                {DAYS.map((d, i) => {
                  ...
                  return (
                    <th key={d} className={`border border-gray-700 px-2 py-2 font-medium min-w-[80px] text-center ${isClosed ? 'text-gray-600' : 'text-gray-300'}`}>
                      <div>{d}</div>
                      <button
...
export default function ScheduleBuilder() {
  ...
  const {
    ...
    republishing, forking, discarding, copying, applyingTemplate, error, violations, hasEdits, hasDraft, liveSchedule,
