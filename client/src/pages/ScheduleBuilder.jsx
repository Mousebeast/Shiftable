          ...
          <tbody>
            <tr>
              ...
              {days.map(date => {
                ...
                const dayCoverage = eventCoverage.filter(ec => ec.date === date);
                return (
                  <td
                    key={date}
    ...
    <div>
      <div className="px-4 py-[18px] border-b border-gray-700 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={`text-xs font-medium ${fillOrder === 'group_first' ? 'text-blue-400' : 'text-amber-400'}`}>
            {fillOrder === 'group_first' ? 'Group Priority' : 'Day Priority'}
          </span>
          <button
            onClick={handleToggleFillOrder}
            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${fillOrder === 'group_first' ? 'bg-blue-500' : 'bg-amber-500'}`}
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4" onClick={() => setShowFillHelp(false)}>
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4 text-sm text-gray-300">
              <div>
                <p className="text-blue-400 font-medium mb-1">Group Priority (recommended)</p>
              ...
              <div>
                <p className="text-amber-400 font-medium mb-1">Day Priority</p>
