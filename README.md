...
## Windows Server install (SSH / headless)

For Windows Servers accessed over SSH where the GUI wizard can't run. Requires [Docker Engine](https://docs.docker.com/engine/install/) installed on the server.

Open a PowerShell session on the server and run:

```powershell
iwr https://raw.githubusercontent.com/Mousebeast/Shiftable/master/setup-docker.ps1 -OutFile setup.ps1; .\setup.ps1
```

> If PowerShell blocks the script, run `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass` first.

The script asks for your restaurant name, a free [DuckDNS](https://duckdns.org) subdomain and token, and your admin account details — then handles the rest: downloading config files, opening firewall ports, obtaining an SSL certificate, and starting all services.

**Prerequisites:** Docker Engine installed and running, ports 80 and 443 reachable from the internet.

---

## Windows & Mac install

Download the setup wizard from the [latest release](https://github.com/Mousebeast/Shiftable/releases/latest):

- **Windows** — `shiftable-setup-windows.exe`
- **macOS (Apple Silicon)** — `shiftable-setup-mac-arm64`
- **macOS (Intel)** — `shiftable-setup-mac-amd64`

The wizard walks through everything — Docker, DuckDNS, SSL certificate, and first-run configuration — through a guided dialog. No command line required.

**Prerequisites:** [Docker Desktop](https://www.docker.com/products/docker-desktop/) installed and running, ports 80 and 443 forwarded on your router.

> **Windows SmartScreen warning:** Windows may show "Windows protected your PC" when you first run the installer. This is normal for new software from small publishers. Click **More info → Run anyway** to proceed.
