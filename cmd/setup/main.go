...
func run() error {
	...
	summary := fmt.Sprintf(
		...
		cfg.restaurantName, cfg.timezone, cfg.weekStart, cfg.duckdnsSub, cfg.adminName, cfg.adminEmail, cfg.installDir,
	)
	if cfg.staging {
...
func collectInputs() (*config, error) {
	...
	// Dates and "today" resolve in the container's local time. Left at UTC for a
	// non-UTC restaurant, the day rolls over at the wrong moment — after roughly
	// 7pm in the Americas the app treats tomorrow as today. Defaulted from the
	// machine running the wizard, which is usually the right answer.
	cfg.timezone, err = ask(
		"What timezone is the restaurant in?\n\n"+
			"Used for dates and for deciding what counts as \"today\".\n"+
			"Use an IANA name, e.g. America/New_York or Europe/London.",
		localTimezone(), false)
	if err != nil {
		return nil, err
	}

	// Every schedule, availability pattern and week event is keyed to the week
	// start, and day_of_week values are offsets from it, so this can only be set
	// here — changing it later would reinterpret every stored week. No in-app
	// setting exists for the same reason.
	weekChoice, err := zenity.List(
		"Which day does your schedule week start on?\n\n"+
			"This is fixed at setup and cannot be changed later.",
		[]string{"Monday", "Sunday"},
		zenity.Title(appTitle),
		zenity.DefaultItems("Monday"),
	)
	if err != nil {
		return nil, err
	}
	cfg.weekStart = strings.ToLower(weekChoice)
...
// localTimezone returns the IANA zone of the machine running the wizard, which
// is the sensible default for the restaurant. Falls back to UTC rather than
// guessing when the platform does not report a name (common on Windows).
func localTimezone() string {
	if tz := os.Getenv("TZ"); tz != "" {
		return tz
	}
	if name, _ := time.Now().Zone(); name != "" && strings.Contains(name, "/") {
		return name
	}
...
func writeEnv(cfg *config, domain string) error {
	...
	weekStart := strings.TrimSpace(cfg.weekStart)
	if weekStart == "" {
		weekStart = "monday"
	}
...
`,
		...
		cfg.duckdnsSub, cfg.duckdnsToken, domain, tz, weekStart,
	)
	if cfg.staging {
