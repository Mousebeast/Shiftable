...
# Timezone
#
# Shiftable resolves "today" and "this week" in the server's local time. If the
# server runs UTC while the restaurant does not, those roll over at the wrong
# moment — after roughly 7pm in the Americas the app starts treating tomorrow as
# today, showing staff the wrong week and rejecting same-day time-off requests.
# Asking here is what makes that correct; there is no in-app setting for it.
DEFAULT_TZ="$(timedatectl show -p Timezone --value 2>/dev/null || cat /etc/timezone 2>/dev/null || echo 'UTC')"
echo ""
echo "  Timezone — used for dates and 'today'. Must match the restaurant's local time."
while true; do
  read -rp "  Timezone [$DEFAULT_TZ]: " TZ_INPUT
  APP_TZ="${TZ_INPUT:-$DEFAULT_TZ}"
  if [[ -f "/usr/share/zoneinfo/$APP_TZ" ]]; then
    break
  fi
  echo "    Not a known timezone. Use an IANA name such as America/New_York or Europe/London."
  echo "    Run 'timedatectl list-timezones' to see the full list."
done

# Week start
#
# Which day the schedule week begins on. Every schedule, availability pattern and
# week event is keyed to this, so it is fixed here and has no in-app setting —
# changing it once real data exists would reinterpret every stored week.
echo ""
echo "  Which day does your schedule week start on?"
echo "    1) Monday (default)"
echo "    2) Sunday"
read -rp "  Choose [1/2]: " WEEK_START_CHOICE
case "${WEEK_START_CHOICE:-1}" in
  2) APP_WEEK_START="sunday" ;;
  *) APP_WEEK_START="monday" ;;
esac
...
echo "  (Press Enter to accept the default — only change this if running"
echo "   multiple Shiftable installs on the same server)"
...
if $DEV_MODE; then
  ...
  case "$CONN_CHOICE" in
    1)
      ...
      while true; do
        read -rp "  Enter domain (e.g. schedule.myrestaurant.com): " DOMAIN
...
PORT=3000
MAX_PORT=3099
while [[ $PORT -le $MAX_PORT ]] && ss -tlnp 2>/dev/null | grep -q ":$PORT "; do
  PORT=$((PORT + 1))
done
[[ $PORT -gt $MAX_PORT ]] && fail "No available port found in range 3000–$MAX_PORT. Free a port and try again."
...
# Both are fixed at install and have no in-app setting, so this is the last
# chance to catch a wrong value before real data is keyed to it.
echo "  Timezone:    $APP_TZ"
echo "  Week starts: $APP_WEEK_START"
echo ""
...
cd "$APP_DIR/client" && npm prune --omit=dev --silent
cd "$APP_DIR"
ok "Client built"
...
if systemctl list-unit-files "${SERVICE_NAME}.service" 2>/dev/null | grep -q "$SERVICE_NAME"; then
  warn "Service $SERVICE_NAME already exists — stopping before reinstall."
